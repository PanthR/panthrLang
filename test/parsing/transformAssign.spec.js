   ...
   it('function calls get converted properly', function() {
      main.parse('names(x)<-5; names(x) <<- 5', function(nodes) {
         nodes.forEach(function(node) {
            ...
            expect(transformedNode.type).to.equal(node.type);
            expect(transformedNode.lvalue.type).to.equal('variable');
            expect(transformedNode.lvalue.id).to.equal('x');
            expect(transformedNode.rvalue.type).to.equal('fun_call');
            ...
            expect(transformedNode.rvalue.args[1].type).to.equal('arg_named');
      ...
      main.parse('names(x)[3]<-5; names(x)[3]<<-5', function(nodes) {
         nodes.forEach(function(node) {
            ...
            expect(transformedNode.rvalue.args[1].type).to.equal('arg_named');
            expect(transformedNode.rvalue.args[1].id).to.equal('value');
            expect(transformedNode.rvalue.args[1].value.type).to.equal('fun_call');
            ...
            expect(transformedNode.rvalue.args[1].value.args[2].type).to.equal('arg_named');
